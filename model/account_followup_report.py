        ...
        for currency, aml_recs in res.items():
            ...
            for aml in aml_recs:
                ...
                lines.append({
                    ...
                    'footnotes': {},
                    'unfoldable': False,
                    'columns': [formatLangDate(aml.date), date_due, aml.late_days] + (not public and [aml.expected_pay_date and (aml.expected_pay_date, aml.internal_note) or ('', ''), aml.blocked] or []) + [amount, interests, allowances, total_due],
                    'blocked': aml.blocked,
                })
...
class account_report_context_followup(models.TransientModel):
    ...
    def get_columns_names(self):
        if self.env.context.get('public'):
            return [_(' Date '), _(' Due Date '), _('Overdue days'), _('Amount'), _('Allowances'), _('Interest'), _(' Total Due ')]
        return [_(' Date '), _(' Due Date '), _('Overdue days'), _(' Expected Date '), _(' Excluded '), _('Amount'), _('Allowances'), _('Interest'), _(' Total Due ')]
