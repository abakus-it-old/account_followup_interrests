...
class res_partner(osv.osv):
    ...
    def get_followup_table_html(self, cr, uid, ids, context=None):
        """ Build the html tables to be included in emails send to partners,
            ...
            :rtype: string
        """
        from . import report as account_followup_print
        ...
        if partner.unreconciled_aml_ids:
            ...
            for currency_dict in final_res:
                ...
                <tr>
                    ...
                    <td>''' + _("Interests") + " (%s)" % (currency.symbol) + '''</td>
                    <td>''' + _("Allowances") + " (%s)" % (currency.symbol) + '''</td>
                    <td>''' + _("Lit.") + '''</td>
                </tr>
                ''' 
                total = 0
                for aml in currency_dict['line']:
                    ...
                    followup_table += strbegin + aml['name'] + strend
                    followup_table += strbegin + (aml['ref'] or '') + strend
                    followup_table += strbegin + str(date) + strend
                    followup_table += strbegin + str(aml['balance']) + strend
                    followup_table += strbegin + str(aml['payments_interests']) + strend
                    followup_table += strbegin + str(aml['payments_allowances']) + strend
                    followup_table += strbegin + block + strend
                    followup_table += "</TR>"

                followup_table += "</table> <br />"

                followup_table += "<table style='border: 1px solid black;'>"

                total = reduce(lambda x, y: x+y['balance'], currency_dict['line'], 0.00)
                total = rml_parse.formatLang(total, dp='Account', currency_obj=currency)
                followup_table += '''<tr><td>''' + _("Amount due") + ''' : </td><td> %s </td></tr>''' % (total)

                p_s_o_i = rml_parse.formatLang(partner.payments_sum_of_interests, dp='Account', currency_obj=currency)
                followup_table += '''<tr><td>''' + _("Total of due interests") + ''' : </td><td> %s </td></tr>''' % (p_s_o_i)

                p_s_o_a = rml_parse.formatLang(partner.payments_sum_of_allowances, dp='Account', currency_obj=currency)
                followup_table += '''<tr><td>''' + _("Total of due allowances") + ''' : </td><td> %s </td></tr>''' % (p_s_o_a)

                p_s_d = rml_parse.formatLang(partner.payments_sum_due, dp='Account', currency_obj=currency)
                followup_table += '''<tr style="border-top: 1px solid black;"><td><b>''' + _("Total due") + ''' : </b></td><td><b> %s </b></td></tr>''' % (p_s_d)
